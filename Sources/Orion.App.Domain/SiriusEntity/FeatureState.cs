    ...
    public bool Equals(FeatureState? other)
    {
        if (!ReferenceEquals((object)this, other))
        {
            if (EqualityContract == other?.EqualityContract)
            {
                ...
                var contentsEq=EqualityComparer<IReadOnlyCollection<FeatureContent>>.Default.Equals(Contents, other.Contents)
                    || (Contents.Count == other.Contents.Count && !Contents.Except(other.Contents).Any());
